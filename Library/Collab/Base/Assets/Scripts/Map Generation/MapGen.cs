...
public class MapGen : MonoBehaviour
{
	...
	public void NewMap()
	{
		if (iFloorNumber != 0)
		{
			...
			gameManager.transform.position = new Vector3(gameManager.transform.position.x, gameManager.transform.position.y - 2.5f, gameManager.transform.position.z);
		...
		if (iMapType == 1)
		{
			...
			for (int i = 0; i < iObjectCount; i++)
			{
				...
				if (nGrid[x, y].bPlaced == false)
				{
					...
					GameObject childObject = Instantiate(obstacles[chosenIndex].gameObject, nGrid[x, y].v3WorldPosition, Quaternion.Euler(0, 0, 0));
					childObject.transform.parent = goTerrainFolder.transform;
					nGrid[x, y].bPlaced = true;
                    ...
                    // exitPos defined earlier  
                    pathSuccess = true;
                    bool rebuildMap = false;

                    // Checking path from player to exit
                    PathRequestManager.RequestPath(playerPos, exitPos, this.MapGenPathSuccess);
                    ...
                    PathRequestManager.RequestPath(enemyPos, playerPos, this.MapGenPathSuccess);
